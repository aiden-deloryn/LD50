...
public class EnemySpawner : MonoBehaviour
{
    ...
    void SpawnEnemy(){
        ...
        Instantiate(enemyPrefab[ChooseRandomEnemy()], new Vector2(randintX, randinty), Quaternion.identity);
    }
    int ChooseRandomEnemy(){
        if ((enemyRank < enemyPrefab.Length-1) && (mutationTime * (enemyRank+1) < timePassed)){
            enemyRank += 1;
            //Debug.Log(enemyRank);
        }
